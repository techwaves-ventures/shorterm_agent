...
# `7777…` or `....` cost ~0.5-1.1 s on this box, with the GIL held throughout —
# and this is a single gunicorn worker, so that is the whole process.
...
#
# Bounding the quantifiers instead (`{1,64}`) was tried and is *not*
# equivalent: it matched a truncated tail of an over-long local part, which
# would store the wrong address on the lead, and dropped addresses whose domain
# label ran past the bound.
...
def _dates(text: str) -> tuple[str, str]:
    ...
    """
    # The `(?<![A-Za-z])` lookbehind keeps this linear. Unbounded, `[a-z]*`
    ...
    # ~4.8 s on this box, with the GIL held throughout. Requiring a month to
    # begin at a letter boundary leaves one viable starting offset per word.
    #
    ...
    # the ones reported. Three near misses are worth keeping in mind if this is
    # ever touched again:
    #
